...
const displayMovements = function (acc, sort = false) {
  ...
  movs.forEach(function (mov, i) {
    ...
    const dislayDate = formatMovementDate(date, acc.locale, acc.currency);
...
const formatCurrency = function (sum, locale, currency) {
...
const formatMovementDate = function (date, locale) {
  const calcDaysPassed = (date, currentDate = new Date()) =>
    Math.round(Math.abs((currentDate - date) / 1000 / 60 / 60 / 24));
  console.log(date);

  const daysPassed = Math.round(calcDaysPassed(date));
...
const startLoginTimer = function () {
  ...
  const updateTimer = function () {
    ...
    if (timer === 0) {
      ...
      labelWelcome.textContent = `Login to see the details.`;
