...
const displayBalance = function (account) {
  const balance = account.movements.reduce((acc, curr, i, arr) => acc + curr);
...
const displaySummary = function (movements, interestRate) {
  const incomes = movements.filter(x => x > 0).reduce((sum, i) => sum + i, 0);
  labelSumIn.textContent = incomes;

  const out = movements.filter(x => x < 0).reduce((sum, i) => sum + i, 0);
  labelSumOut.textContent = -out;

  const interest = movements
    .filter(x => x > 0)
    .map(x => (x * interestRate) / 100)
    .filter(i => i >= 1)
...
btnLogin.addEventListener('click', function (e) {
  ...
  e.preventDefault(); //default html behaivour for submit buttn is to reload page
...
function asd(input) {
  input.map(x => (x.length > 1 && x === x.toLowerCase() ? x.toUpperCase() : x));
}
