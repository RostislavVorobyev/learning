...
function getRecomendedValue(weight) {
  return weight ** 0.75 * 28;
}

function isOvereater({ recommendedFood, curFood }) {
  curFood > recommendedFood * 0.9 && curFood < recommendedFood * 1.1;
}

dogs.forEach(cur => (cur.recommendedFood = getRecomendedValue(current.weight)));

dogs.find(x => x.owners.includes('Sarah'));
